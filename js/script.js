        ...
        function isDropboxEnabled() {
            try { return localStorage.getItem('ops_dbx_enabled') === '1'; } catch(e) { return false; }
        }
        ...
        function downloadJson() {
            ...
            const { token } = getDropboxConfig();
            const useCloud = isDropboxEnabled();
            if (useCloud) {
                if (!token) { showInfo('未登录 Dropbox，请先在 Dropbox 配置中完成登录。', '提示'); return; }
                // 上传到 Dropbox
        ...
        function setupUIEvents() {
            ...
            document.getElementById('dropbox-config-btn')?.addEventListener('click', openDropboxModal);
        ...
        function openDropboxModal() {
            const wrap = document.getElementById('modal-dropbox');
            ...
            const enabledEl = document.getElementById('modal-db-enabled');
            const saveBtn = document.getElementById('modal-db-save');
            const cancelBtn = document.getElementById('modal-db-cancel');
            const closeBtn = document.getElementById('modal-db-close');
            const authBtn = document.getElementById('modal-db-authorize');
            const loginBtn = document.getElementById('modal-db-login');
            ...
            if (!fileRow && bodyEl) {
                ...
                fileRow.id = 'modal-db-file-row';
                const label = document.createElement('label');
                label.textContent = '选择JSON文件（Dropbox 应用目录 Prompt Studio）';
                ...
                fileRow.appendChild(rowInner);
                bodyEl.appendChild(fileRow);
            }
            // 预填文件选择
            if (fileSel) {
                const preSel = getDropboxSelectedFilename();
                fileSel.innerHTML = '';
            ...
            const onSave = () => {
                try {
                    ...
                    if (v) localStorage.setItem('ops_dbx_filename', v);
                } catch(e) {}
                cleanup();
            };
            const onCancel = () => cleanup();
            ...
            function cleanup() {
                ...
                cancelBtn.removeEventListener('click', onCancel);
                closeBtn.removeEventListener('click', onCancel);
                authBtn.removeEventListener('click', onAuthorize);
